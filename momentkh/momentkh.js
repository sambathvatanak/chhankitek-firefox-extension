...
;(function (global, factory) {
  ...
  function toLunarDateParameter(format,date) {

    //let target = date;
    let target = moment(date);
    let result = findLunarDate(target);
    let c = formatKhmerDate({day: result.day,month: result.month,moment: target}, format);
    return formatKhmerDate({
      day: result.day,
      month: result.month,
      moment: target
  ...
  function toNormalDate(date) {
    ...
    let month = str[1];
    let year = str[0];
    let result  = findNormalDate(day,month,year);
    return result;
  }

  function findNormalDate(day,month,year){
    day = normalDayNew(day);
    month = normalMonth(month);
    year = normalYear(year);
    return "ថ្ងៃទី" + day + " ខែ" + month + " ឆ្នាំ" + year;
  }

  function normalDayNew(day){
    let digits = (""+day).split("");
    let one = normalDay(digits[0]);
    let two = normalDay(digits[1]);
    return one + two;
  }
  ...
  function normalYear(year){
    ...
    let one = normalDay(digits[0]);
    let two = normalDay(digits[1]);
    let three = normalDay(digits[2]);
    let four = normalDay(digits[3]);
  ...
  Moment.fn.toLunarDateParameter =
    Moment.fn.toKhDateParameter =
      Moment.fn.tokhdateparameter =
        toLunarDateParameter;

 Moment.fn.toNormalDate =
    Moment.fn.toNormalDate =
      Moment.fn.tonormaldate =
        toNormalDate;
